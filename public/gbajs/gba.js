function GameBoyAdvance() {
  ...
  // throttle = ms -> higher throttle = lower ops / s
...
GameBoyAdvance.prototype.saveLocal = function (rom, savename) {
  ...
  try {
    ...
    var key = this.SYS_ID + '.save';
    if (!storage[key]) storage[key] = '{}';

    var parsed_storage = JSON.parse(storage[key]);
    if (!parsed_storage[rom]) parsed_storage[rom] = {};
    parsed_storage[rom][savename] = this.encodeSavedata(sram.view);

    storage[key] = JSON.stringify(parsed_storage);
    return true;
  } catch (e) {
...
GameBoyAdvance.prototype.loadLocal = function(rom, save) {
  try {
    var storage = window.localStorage;
    var key = this.SYS_ID + '.save';
